...
print_usage()
  {
  fprintf(stderr,"usage: dewin (-can) (-f [filter file]) [ch no.(in hex)] ([input file])\n");
...
read_one_sec(ptr,sys_ch,abuf)
     unsigned char *ptr; /* input */
     long sys_ch;    /* sys_ch = sys*256 + ch */
     register long *abuf;/* output */
{
   ...
   dp=ptr+10;
   ddp=ptr+mklong(ptr); /* Don't SWAPL */
...
main(argc,argv)
  ...
  char *argv[];
  {
  int i,j,k,sec,re,sysch,mainsize,sr,sr_save,c,form,nofill,zero,
  ...
  form=nofill=filter_flag=0;
  for(i=0;i<MAX_FILT*4;++i)
    uv[i]=0.0;

  while((c=getopt(argc,argv,"chnaf:"))!=EOF){
     switch(c){
      case 'a':
    ...
    }  /*  End of "while((c=getopt(argc,argv,"chnaf:"))!=EOF){" */
  ...
  while(mainsize=read_data(&mainbuf,f_main)){
     ...
     if(sr_save==0){
	fprintf(stderr,"%04X  %d Hz  ",sysch,sr);
