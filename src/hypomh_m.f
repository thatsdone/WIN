C  hypomh_m.f : This is a version which permits multiple velocity
C               structure. Each station can have a velocity model
C               which is uesed for calculation of travel times
C               and their partial derivatives with spatial coordinate
C             : This version was modified from S.Sakai's code on
C               June 9, 1998 by N. Hirata & S.Sakai
C
...
C        CCP = 1.0D-2 modified by coco             2/17/98
C        Structure for every stations              6/ 5/98
C
      PROGRAM HYPOMH
C
...
C
      SUBROUTINE HYPINI
      IMPLICIT REAL*8 (A-H,O-Z)
      COMMON /CNTP/ EPS1,EPS2,EPS3,LIT1,LIT2
      EPS1=1.0D-8
      EPS2=1.0D-14
      EPS3=1.0D-6
      LIT1=30
      LIT2=10
      WRITE(21,2100)
 2100 FORMAT(' HYPOCENTER LOCATION USING A BAYESIAN APPROACH ver.6/9/98'/
     145(1H-))
      RETURN
      END
C
...
C              PUT JUDG=4 AND RE-CALCULATION: PE(I) OR SE(I)=-2.0
      ...
      CHARACTER*10 SA,VNAME,VST
      DIMENSION DL(100),AZ(100),TA(100),TB(100),BMAG(100)
      COMMON /STRC/ Y(22),VR(22),VLG(21),V(21),N1
      COMMON /STRC1/ VST
      COMMON /DATA/ SC(3,100),PT(100),PE(100),ST(100),SE(100),
     1              FMP(100),AMP(100),NE,ND,IYR,MNT,IDY,IHR,MIN
      COMMON /DATA1/ SA(100),VNAME(100)
      COMMON /DATF/ APT(100),AST(100),NPD,NSD
      COMMON /DATF1/ POLA(100)
      COMMON /STRT/ XM0(3),EX0(3)
      COMMON /RSLT/ XM1(3),EX1(3),H(3,3),RSL(3),COT,EOT,
     1              RPT(100),RST(100),TAG(100),TBG(100)
...
C
      ...
      ENDIF
C
      WRITE(22,2249) SA(I),POLA(I),DL(I),AZ(I),TA(I),TB(I),
     1   PT(I),PE(I),RPT(I),ST(I),SE(I),RST(I),FMAG,BMAG(I),VNAME(I)
      WRITE(21,2249) SA(I),POLA(I),DL(I),AZ(I),TA(I),TB(I),
     1   PT(I),PE(I),RPT(I),ST(I),SE(I),RST(I),FMAG,BMAG(I),VNAME(I)
      WRITE( 6,2249) SA(I),POLA(I),DL(I),AZ(I),TA(I),TB(I),
     1   PT(I),PE(I),RPT(I),ST(I),SE(I),RST(I),FMAG,BMAG(I),VNAME(I)
   15 CONTINUE
...
C 2220 FORMAT(2(F8.3,1X),F8.3,2(F9.3,2X),F8.3)
 ...
 2240 FORMAT(2X,I2,1X,A4,1X,3(I2,1X,'(',F5.1,'%',1X,')',1X))
C 2250 FORMAT(A5,A2,4F6.1,8F6.2,F6.1,E10.3,F5.1)
 2250 FORMAT(A10,1X,A2,F8.3,3F6.1,2(F7.3,F6.3,F7.3),E10.3,F5.1)
C 2249 FORMAT(A5,A2,4F6.1,2(F6.2,F5.2,F6.2),E10.3,F5.1,1x,A4)
 2249 FORMAT(A10,1X,A2,F8.3,3F6.1,2(F7.3,F6.3,F7.3),E10.3,F5.1,1X,A4)
...
C     INVERSION OF ARRIVAL TIME DATA FOR HYPOCENTER COORDINATES
     ...
     1              RPT(100),RST(100),TAG(100),TBG(100)
      COMMON /CNTP/ EPS1,EPS2,EPS3,LIT1,LIT2
      DIMENSION A(100,3),B(3,3),XMC(3),RVX(3),VXM(3),VPS(100),XW(3)
      DIMENSION CP(100,3),CS(100,3),DP(3,3),DS(3,3),FP(100,100),
     1              FS(100,100)
      DIMENSION VPT(100),VST(100),TPT(100),ANG(22),TRV(22),TW(100),
     1              BNG(100)
      LL=0
      LM=20
      JM=10
...
C
  ...
  300 JJ=0
  200 CONTINUE
C CALL WHERE(XM1(3),LN)
      DO 140 I=1,ND
      XX=XM1(1)-SC(1,I)
      YY=XM1(2)-SC(2,I)
      RR=DSQRT(XX*XX+YY*YY)
      IF(RR.LT.EPS3) RR=EPS3
      CALL VLSTRS (SA(I))
C
C     debug : 6/9/98 N.H.
C
C      write(21,*) 'STRNAME:',STRNAME,'NN=',NN
C      do 141 ii=1,21
C      write(21,*) 'Y,V:',Y(ii),VR(ii)
C 141  CONTINUE
C      write(21,*) 'VLCT,ZMIN,ZMAX:', VLCT,ZMIN,ZMAX
C
      VNAME(I)=STRNAME
      CALL TRAVEL ( RR, XM1(3), SC(3,I), NP, ANG, TRV, BNG)
C
C
C      write(21,*) SA(I),RR,XM1(3),SC(3,I),NP
C
C
      IF (NP.EQ.0) THEN
                   WRITE(21,2110) I
...
C
C    for debug by N.H. 6/9/98
C
C      write(21,*) 'SN,XX,RR,VRE:',SN,XX,RR,VRE
C
C
      A(I,1)=SN*XX/RR/VRE
      A(I,2)=SN*YY/RR/VRE
  140 A(I,3)=-CN/VRE
      ...
      WST=VST(I)*RST(I)
C
C for debug 6/9/98 by N.H.
C
C      write(21,*) 'I,RPT(I),RST(I),WPT,WST:' ,I,RPT(I),RST(I),WPT,WST
C      write(21,*) 'PT(I),TPT(I):',PT(I),TPT(I)
C
C
...
C     ARE ADDED TO THE PE(I) AND SE(I).
     ...
     1              FMP(100),AMP(100),NE,ND,IYR,MNT,IDY,IHR,MIN
      ...
      COMMON /DATF1/ POLA(100)
      COMMON /OCOD/ ALAT0,ALNG0,DEPT0
      DIMENSION PT1(100),PE1(100),ST1(100),SE1(100),POLA1(100),
     1             SA1(100),ALAT(100),ALNG(100),AHGT(100),XST(100),
     1             YST(100),STCP(100),STCS(100)
C
      NE=0
...
C 1310 FORMAT(A4,1X,A1,2(F8.3,F6.3),F6.1,E9.2,2F11.5,I7,2F7.3)
...
C     modified for multi-structure model,  6/4/98 N.H.
C     name of individual structure can be 6 characters
C
      IMPLICIT REAL*8(A-H,O-Z)
      CHARACTER*10 VST,VST0
      COMMON /STRC/ Y(22),VR(22),VLG(21),V(21),N1
      COMMON /STRC1/ VST
      COMMON /STRC2/ VLCT,ZMIN,ZMAX
      COMMON /STRT1/ EX1(3)
C
C    following COMMON statements are added to store multiple-structure
C    model,
C      6/4/98 N.H.
C
C      modified no 6/9/98 N.H
C
      COMMON /STRC0/ Y0(22,100),VR0(22,100),VLG0(21,100),V0(21,100),
     1               N10(100)
      COMMON /STRC01/ VST0(100),NVST
      COMMON /STRC02/ VLCT0(100),ZMIN0(100),ZMAX0(100)
      COMMON /STRT01/ EX10(3,100)
C
C     NVST: number of structure models, which should be less or equal to 100
C
      COMMON /OCOD/ ALAT0,ALNG0,DEPT0
      DIMENSION TH0(21,100)
      READ(11,1101) ALAT0,ALNG0,DEPT0
 1101 FORMAT(3F10.0)
C
C    read structure models
C
      MAXSRT = 100
      DO 5 NSTR = 1,MAXSRT
      READ(11,'(I5,2X,A4)',END=6) NN,VST0(NSTR)
C
C
C     IF( NN .LT. 0.5 ) GO TO 6
      IF( NN .LT. 1   ) GO TO 6
      IF(VB.LE.VA) VLCT0(NSTR)=0.0D0
      IF(VB.GT.VA) THEN
         VLCT0(NSTR)=(VB-VA)/DLOG(VB/VA)
      ENDIF
      N10(NSTR) =NN+1
      READ(11,1100) (VR0(1+I,NSTR),I=0,N10(NSTR))
      READ(11,1100) (TH0(I,NSTR),I=1,N10(NSTR))
 1100 FORMAT(7F10.0)
C      write(6,*) (VR0(1+I,NSTR),I=0,N10(NSTR))
C      write(6,*) (TH0(I,NSTR),I=1,N10(NSTR))
      IF(NSTR.LT.2) READ(11,'(4F10.0)') EOT,EX1(2),EX1(1),EX1(3)
    5 CONTINUE
    6 NVST = NSTR - 1
C      write(6,*) 'NVST =',NVST
      IF(NVST .EQ. 0 ) THEN
         WRITE(21,2130)
 2130    FORMAT(1H ,"NO STRUCTURE IS SPECIFIED")
          STOP
      ENDIF
c
      DO 12 NSTR = 1, NVST
      DO 10 I=1,N10(NSTR)
   10 VLG0(I,NSTR)=(VR0(1+I,NSTR)-VR0(1+I-1,NSTR))/TH0(I,NSTR)
      Y0(01+0,NSTR)=0.0D0
      DO 20 I=1,N10(NSTR)
   20 Y0(1+I,NSTR)=Y0(1+I-1,NSTR)+TH0(I,NSTR)
      V0(1,NSTR)=VR0(1+0,NSTR)/VLG0(1,NSTR)
      DO 30 I=2,N10(NSTR)
      J=I-1
   30 V0(I,NSTR)=(VLG0(J,NSTR)*V0(J,NSTR)+(VLG0(J,NSTR)-VLG0(I,NSTR))*
     1Y0(1+J,NSTR))/VLG0(I,NSTR)
      ZMIN0(NSTR)=-VR0(1+0,NSTR)/VLG0(1,NSTR)+1.0D-1
      ZMAX0(NSTR)=Y0(1+N10(NSTR),NSTR)-0.1D0
C
      WRITE(21,2100) VST0(NSTR)
 2100 FORMAT(/' ***** VELOCITY STRUCTURE (',A6,') *****'/9X,
     1'I',4X,'Y(I)',6X,'VR(I)',5X,'ALPHA(I)',4X,'VLG(I)')
      DO 60 I=1,N10(NSTR)
      J=I-1
      WRITE(21,2110) J,Y0(1+J,NSTR),VR0(1+J,NSTR)
  60  WRITE(21,2120) V0(I,NSTR),VLG0(I,NSTR)
      WRITE(21,2110)
     1 N10(NSTR),Y0(1+N10(NSTR),NSTR),VR0(1+N10(NSTR),NSTR)
 ...
 2120 FORMAT(30X,2D12.3)
   12 CONTINUE
      ...
      END
c
      SUBROUTINE VLSTRS(STRNAME)
C
C    Select a model, whose name is STRNAME,
C     from multiple-structure,    6/4/98 N.H.
C
      IMPLICIT REAL*8(A-H,O-Z)
      CHARACTER*10 VST,STRNAME
      CHARACTER*10 VST0
      COMMON /STRC/ Y(22),VR(22),VLG(21),V(21),N1
      COMMON /STRC1/ VST
      COMMON /STRC2/ VLCT,ZMIN,ZMAX
...
C
  ...
  30         CONTINUE
              Y(N1+1) = Y0(N1+1, NSTR)
              VR(N1+1) = VR0(N1+1, NSTR)
C
             VLCT    = VLCT0(NSTR)
             ZMIN    = ZMIN0(NSTR)
             ZMAX    = ZMAX0(NSTR)
             DO 35 I  = 1,3
  35         EX1(I)  = EX10(I,NSTR)
C
             ENDIF
  20  CONTINUE
      RETURN
