...
struct station_data {
   ...
   double  amp;        /* MAXIMUM AMPLITUDE DATA (IN m/s, 0.0 FOR NO USE) */
...
/***** Read arrival time data *****/
   ...
   for(i=0; i<na; ++i){
      fgets(txtbuf, LINELEN, fp_13);
      strncpy(sta[i].sa1,txtbuf,10); sta[i].sa1[10]='\0';
      strncpy(sta[i].pola1,txtbuf+11,1); sta[i].pola1[1]='\0';
...
/*   a = 1.7320508;*/
   ...
   for(i=0; i<nd; ++i){
      ...
      pe2=calc[i].pe*calc[i].pe+(calc[i].pt-otm)/1.0e2*(calc[i].pt-otm)/1.0e2;
      se2=calc[i].se*calc[i].se+(calc[i].st-otm)/5.0e1*(calc[i].st-otm)/5.0e1;
      var = pe2*c*c+se2*b*b;
      sot += otm/var;
      svr += 1.0/var;
      k++;
