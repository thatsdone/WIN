/*-
  $Id: hypomhc2.c,v 1.1.2.3 2010/12/28 12:55:41 uehira Exp $
...
main(int argc, char *argv[])
{
  ...
  if (NULL == (strcs.vr = (double *)win_xmalloc(sizeof(double) * (strcs.n1 + 1))))
    memory_error();
  if (NULL == (strcs.y = (double *)win_xmalloc(sizeof(double) * (strcs.n1 + 1))))
    memory_error();
  if (NULL == (strcs.vlg = (double *)win_xmalloc(sizeof(double) * (strcs.n1))))
    memory_error();
  if (NULL == (strcs.v = (double *)win_xmalloc(sizeof(double) * (strcs.n1))))
    memory_error();
...
line150:
  ...
  if (NULL == (bng = (double *)win_xmalloc(sizeof(double) * (strc.n1 + 1))))
    memory_error();
  /* S wave */
  if (NULL == (angs = (double *)win_xmalloc(sizeof(double) * (strcs.n1 + 1))))
    memory_error();
  if (NULL == (trvs = (double *)win_xmalloc(sizeof(double) * (strcs.n1 + 1))))
    memory_error();
  if (NULL == (bngs = (double *)win_xmalloc(sizeof(double) * (strcs.n1 + 1))))
    memory_error();
  /* special structure */
  if (smode) {
    /* P wave */
