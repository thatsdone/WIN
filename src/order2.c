/*  $Id: order2.c,v 1.1.2.2 2001/11/19 10:44:11 uehira Exp $  */
...
advance(shm,shp,c_save,size,tm,shp_busy_save)
  struct Shm *shm;
  ...
  char *argv[];
{
  ...
  unsigned long uni;
  int tm[6],tm_out[6],tm_bottom[6],i,c_save_in,shp_in,
    sec,sec_1,size,sizej,shmid_in,shmid_out,shmid_late,size_in,shp,c_save,n_sec,no_data;
  unsigned char *ptr,*ptr_save,*ptw,*ptw_save,*ptw_late,tbuf[100];
  ...
  if(argc<7){
    printf(
" usage : '%s [shm_key_in] [shm_key_out] [shm_size(KB)] [limit_sec] [shm_key_late] [shm_size_late(KB)] ([log file])'\n",
      progname);
    exit(0);
  }
  ...
  n_sec=atoi(argv[4]);
  shm_key_late=atoi(argv[5]);
  sizej=atoi(argv[6])*1000;
  if(argc>6) strcpy(logfile,argv[7]);
  else *logfile=0;
...
reset:
  ...
  while(1){
    ...
    ptw+=4;
    ptw_late=shm_late->d+shm_late->p;
    ...
    do{
      ...
      if(shp==shp_busy_save) timeout_flag=1;
      if(timeout_flag){
	if(time_cmp(tm,tm_out,6)==-1){
	  /* output late data */
	  ptr=shm_in->d+shp;
	  memcpy(ptw_late,ptr,size);
	  uni=time(NULL);
	  ptw_late[4]=uni>>24;
	  ptw_late[5]=uni>>16;
	  ptw_late[6]=uni>>8;
      ...
      while((sec=advance(shm_in,&shp,&c_save,&size,tm,&shp_busy_save))==(-1));
      if(sec==(-2)){
        ptr=shm_in->d+shp+8;
...
"passing %02X%02X%02X.%02X%02X%02X:%02X%02X(out=%02d%02d%02d.%02d%02d%02d)",
      ...
      while((sec_1=advance(shm_in,&shp_in,&c_save_in,&size_in,tm_bottom,NULL))<=0){
	if(shp_in==shp_busy_save) shp_busy_save=(-1);
        if(sec_1==(-1)) continue;
        else if(sec_1==0){
          sleep(1);
          continue;
